                        ...
                        )
{
  ...
  pip_value <- getPipValue('EUR_USD', auth_id=auth_id, account_id=account_id)
  ...
  url <- paste0('https://api-fxpractice.oanda.com/v1/accounts/',
                account_id, '/orders')
                          ...
                          )
{
  ...
  url <- paste0('https://api-fxpractice.oanda.com/v1/candles?', options)
 ...
 ##  Get Price  ## 
#################
getPrice <- function(instrument='EUR_USD', auth_id)
{
  ##  Athorization
  auth <- c(Authorization=paste('Bearer', auth_id))
  ##  Get Current price:
  url <- paste0('https://api-fxpractice.oanda.com/v1/prices?instruments=', 
                instrument)
 ...
 ##  Get pip value  ##
#####################
getPipValue <- function(instrument='EUR_USD', account_id, auth_id)
{
  auth <- c(Authorization=paste('Bearer', auth_id))
  url <- paste0('https://api-fxpractice.oanda.com/v1/instruments?accountId=',
 ...
 ##  Get Open Orders  ##
#######################
getPositions <- function(instrument, account_id, auth_id)
{
  auth <- c(Authorization=paste('Bearer', auth_id))
  url <- paste0('https://api-fxpractice.oanda.com/v1/accounts/',
                    account_id, '/positions?instrument=', instrument)
 ...
 ##  Close Orders  ##
####################
closeAllOrders <- function(instrument, account_id, auth_id)
{
  auth <- c(Authorization=paste('Bearer', auth_id))
  url <- paste0('https://api-fxpractice.oanda.com/v1/accounts/',
                account_id, '/positions/', instrument) 
